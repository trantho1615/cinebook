...
- [ ] **Step 8b: Sửa lỗi ranh giới module của chính plan này**

`RefundUseCase` ở Step 7 import `com.cinebook.booking.infra.AuditLogger` — `ModuleBoundaryTest`
bắt được ngay (4 vi phạm: field, tham số constructor, lời gọi method, đọc hằng enum).
Đây là lần thứ tư luật ArchUnit bắt lỗi trong plan do chính tôi viết.

Cách sửa: chuyển `AuditLogger` sang `com.cinebook.shared.audit`. Bảng `audit_log` vốn
hoàn toàn tổng quát (`aggregate_type`, `aggregate_id`, `action`, `actor_type`) và
`payment` phải ghi vào nó khi hoàn tiền — cùng lý do với `shared/outbox`. Không dựng
interface `booking.api` cho việc này, vì audit không phải việc của module booking.
File migration `V7__booking.sql` giữ nguyên tên: migration chỉ thêm chứ không sửa.

Cập nhật import ở `HoldSeatsUseCase`, `CancelBookingUseCase`, `ConfirmBookingUseCase`
và `RefundUseCase`.

- [ ] **Step 8c: Kiểm chứng bẫy rollback-only là có thật**

`ConfirmBookingUseCase.confirm` đã mang `REQUIRES_NEW` từ Task 3, nên chưa test nào
từng thấy cái bẫy này cắn. Tạm đổi về `@Transactional` mặc định rồi chạy
`LateWebhookTest`: 3/4 test đỏ với đúng thông báo plan dự đoán —
`UnexpectedRollbackException: Transaction silently rolled back because it has been
marked as rollback-only`. Khôi phục thì xanh lại.
...
- [ ] **Step 7b: Bổ sung test cho chính lời hứa của outbox**

Năm test trên **không** test điều quan trọng nhất: event và thay đổi nghiệp vụ cùng
commit hoặc cùng rollback. Đã kiểm chứng bằng thực nghiệm: tạm thêm
`@Transactional(propagation = REQUIRES_NEW)` vào `OutboxWriter.write` — event khi đó
sống sót qua rollback của giao dịch nghiệp vụ, nhưng **cả 5 test vẫn xanh**. Thêm test
thứ sáu `event_bi_rollback_cung_voi_thay_doi_nghiep_vu`: mở `TransactionTemplate`,
update `bookings`, gọi `outbox.write(...)`, rồi ném exception; khẳng định
`count(*) FROM outbox_events` bằng 0 và đơn vẫn ở `PENDING`. Test này đỏ đúng như mong
đợi với bản bị phá, xanh khi khôi phục.
...
Kỳ vọng: BUILD SUCCESS, 112 test (111 theo kế hoạch + 1 test ở Step 7b).
...
Kỳ vọng: BUILD SUCCESS, 119 test (118 theo kế hoạch + 1 test thêm ở Task 4).
...
- [ ] `mvn -B clean verify` xanh, 119 test.
